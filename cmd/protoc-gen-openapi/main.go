...
func main() {
	conf := generator.Configuration{
		...
		NoRecursiveSchemaNaming: flags.Bool("no_recursive_schema_naming", false, `if "true", only include the immediate parent message name in schema names instead of the full message hierarchy. This restores the legacy naming behavior where e.g. A.B.C.Request becomes C_Request instead of A_B_C_Request`),
